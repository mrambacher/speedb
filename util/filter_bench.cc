...
using Policy = std::shared_ptr<const ROCKSDB_NAMESPACE::FilterPolicy>;

std::pair<Policy, int> CreateFilterPolicy() {
  ...
  int bloom_idx =
      ROCKSDB_NAMESPACE::BloomLikeFilterPolicy::GetAllFixedImplIndex(
          policy->Name());
  if (FLAGS_use_plain_table_bloom) {
