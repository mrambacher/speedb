...
gen-pc:
	-echo 'prefix=$(PREFIX)' > $(PROJECT_NAME).pc
	-echo 'exec_prefix=$${prefix}' >> $(PROJECT_NAME).pc
	-echo 'includedir=$${prefix}/include' >> $(PROJECT_NAME).pc
	-echo 'libdir=$(LIBDIR)' >> $(PROJECT_NAME).pc
	-echo '' >> $(PROJECT_NAME).pc
	-echo 'Name: $(PROJECT_NAME)' >> $(PROJECT_NAME).pc
	-echo 'Description: An embeddable persistent key-value store for fast storage' >> $(PROJECT_NAME).pc
	-echo Version: $(shell ./build_tools/version.sh full) >> $(PROJECT_NAME).pc
	-echo 'Libs: -L$${libdir} $(EXEC_LDFLAGS) -l$(LIBNAME:lib%=%)' >> $(PROJECT_NAME).pc
	-echo 'Libs.private: $(PLATFORM_LDFLAGS)' >> $(PROJECT_NAME).pc
	-echo 'Cflags: -I$${includedir} $(PLATFORM_CXXFLAGS)' >> $(PROJECT_NAME).pc
	-echo 'Requires: $(subst ",,$(ROCKSDB_PLUGIN_PKGCONFIG_REQUIRES))' >> $(PROJECT_NAME).pc
