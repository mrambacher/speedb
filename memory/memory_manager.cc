...
SpdbMemoryManager::SpdbMemoryManager(const SpdbMemoryManagerOptions &options)
  ...
  // URQ - This method is called under the lock of the mutex in all other cases.
  // Is it ok without here?
  RecalcConditions();
};
...
void SpdbMemoryManager::SetMemSizeDelayFactor(size_t factor) {
  if (memsize_delay_factor_ != factor) {
    ...
    // URQ - Why is this needed? The delay_enforcer_->GetDelayFactor() should be
    // the largest of all cf-s and memsize_delay_factor_ anyway?
...
bool SpdbMemoryManager::InitiateFlushRequest() {
  ...
  for (auto client : clients_) {
    ...
    for (auto db : flush_disabled_db_) {
      if (db == client->db()) flush_disabled = true;
    }
    if (!flush_disabled) {
      // URQ - Why use here the GetUnFlushDataSize() and not the
      // GetImMutableDataSize() URQ - If atomic flush is set, then every flush
      // will be for the entire DB => shouldn't we check if atomic flush is on
      // and set the flush_size accordingly? This needs reconsideration and may
      // be a bug. The intention was to get the memtables that haven't been
      // marked for flush.
      size_t flush_size =
...
// URQ - Note that this is called when the memtable is actually deleted. That
// may be not immediately when flushing it completed but may be deferred in case
// the memtable was put on the history list (memlist_history_)

// Note that we must first remove the freed memory from the dirty memory amount
// and only then notify that a flush has completed. This will make sure that
// when deciding if another flush is needed, the dirty memory is up to date
...
// URQ - The size is not actually in use and is passed as 0 when the flush
// starts, is that on purpose?
void SpdbMemoryManager::FlushStarted(const size_t size) {
  std::unique_lock<std::mutex> lck(mutex_);
  size_pendding_for_flush_ += size;
  // URQ - The flush may have started not due to us => we should NOT decrease
  // n_scheduled_flushes_ This is true but we will not decrease the
  // n_scheduled_flushes_ below 0
  if (n_scheduled_flushes_ > 0) n_scheduled_flushes_--;
  n_running_flushes_++;
}

// URQ - The memtable may still be alive and consume memory (in case it was
// placed on the memlist_history_ list)
    ...
    while (!clients_need_flush_.empty()) {
      SpdbMemoryManagerClient *client = nullptr;
      {
        ...
        if (!clients_need_flush_.empty()) {
          ...
          // URQ - And we didn't Ref() before putting on the clients_need_flush_
          // list => It may have been dropped already Is it safe to call Ref()
          // on it?
      ...
      if (client) {
        ...
        // If this returns true => cf was deleted => Remove the client. Never
        // try to ref its cf again
...
// Update / re-set the paramteres so that the existing decision to flush based
// on write buffer full will not be taken => leave the control for flush with
// the WBM URQ - Does this method also prevents
// WaitUntilFlushWouldNotStallWrites() from waiting? - NO URQ - This overwrites
// the values set by the user. A bit problematic IMO
